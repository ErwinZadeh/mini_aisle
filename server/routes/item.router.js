...
router.get('/', rejectUnauthenticated, (req, res) => {
  ...
  let queryText =
    `SELECT "id", "status", "item_name", "amount", "amount_unit", "category", "store" FROM "item" WHERE ${req.user.id} = "user_id" ORDER BY "store", "category", "item" ASC;`;
...
router.post('/', (req, res) => {
  ...
  let queryText = `INSERT INTO "item" ("item_name", "amount", "amount_unit", "category", "store", "user_id")
                   VALUES ($1, $2, $3, $4, $5, $6)  ;`;
  pool.query(queryText, [newItem.itemName, newItem.amountNumber, newItem.amountUnit, newItem.category, newItem.shoppingStore, newItem.userID])
    .then(result => {
      res.sendStatus(201);
    })
    .catch(error => {
